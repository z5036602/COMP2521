...
static tree_node *rotate_left (tree_node *curr)
{
	...
	size_t tmp_size = curr->size;
	curr->right = rotated_left->left;
	size_t size_left = (curr->left == NULL)? 0:curr->left->size;
	size_t size_right = (curr->right == NULL)? 0:curr->right->size;
	curr->size = size_left+size_right+1;
	rotated_left->left = curr;
	size_left = (rotated_left->left == NULL)? 0:rotated_left->left->size;
	size_right = (rotated_left->right == NULL)? 0:rotated_left->right->size;
	rotated_left->size = size_left+size_right+1;
	return rotated_left;
}
...
static tree_node *rotate_right (tree_node *curr)
{
	...
	size_left = (rotated_right->left == NULL)? 0:rotated_right->left->size;
	size_right = (rotated_right->right == NULL)? 0:rotated_right->right->size;
	rotated_right->size = size_left+size_right+1;
...
static tree_node *partition (tree_node *curr, size_t pos)
{
	...
	size_t left_subtree_size = (curr->left == NULL)? 0:curr->left->size;
...
void white_box_tests (void)
{
	...
	tree * my_tree = tree_new(NO_REBALANCE);
	//assert(1==2);
	char * a1 = "5fdsa";
	char * a2 = "5fd";
	char * a3 = "a";
	...
	char * a7 = "ewrq";
	char * a8 = "ewq";
	tree_insert (my_tree, a1);
	tree_insert (my_tree, a2);
	tree_insert (my_tree, a3);
	tree_insert (my_tree, a4);
	tree_insert (my_tree, a5);
	tree_insert (my_tree, a6);
	tree_insert (my_tree, a7);
	tree_insert (my_tree, a8);
	assert(my_tree->root->size == 8);
	
	my_tree->root = rotate_left (my_tree->root);
	assert(my_tree->root->size == 8);
	my_tree->root = rotate_left (my_tree->root);
	assert(my_tree->root->size == 8);
	my_tree->root = rotate_left (my_tree->root);
	...
	my_tree->root = rotate_left (my_tree->root);
	assert(my_tree->root->size == 8);
	my_tree->root = rotate_right (my_tree->root);
	assert(my_tree->root->size == 8);
		my_tree->root = rotate_right (my_tree->root);
	assert(my_tree->root->size == 8);
		my_tree->root = rotate_right (my_tree->root);
